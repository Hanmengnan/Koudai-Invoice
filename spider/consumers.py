...
class ChatConsumer(WebsocketConsumer):
    def connect(self):
        ...
        if identify(self.scope) == True:
            ...
            if int(dict(self.scope['headers'])[b'cmd'].decode('utf-8')) == 101:  # ask_with_id
                ...
                self.openid = Wgetid(dict(self.scope['headers'])[
                                         b'code'].decode('utf-8'))
            ...
            if int(dict(self.scope['headers'])[b'cmd'].decode('utf-8')) == 115:
                # 通过openid创建DB类，交互数据库操作
                self.openid = Agetid(dict(self.scope['headers'])[
                                         b'code'].decode('utf-8'))
                self.DB = DB(self.openid , '39.105.217.150')
            first_using = self.DB.find_and_insert_openid(self.openid)
            user_mode = {'cmd': 210 , 'address': self.DB.check_email() ,
                         'vip': self.DB.check_vip()}
            # 用户第一次访问
            if not first_using:
    ...
    def receive(self, text_data):
        ...
        # 发票夹存入数据
            ...
            if kindsof(temp_dict) == None:
                ...
                if res == None:
                    ...
                    search_res = self.DB.search_invoice_from_user(
                        jsons['fp_dm'] , jsons['fp_hm'])  # 检查user表里有没有这张发票
                    if not search_res:
                        self.DB.add_invoice(jsons['fp_dm'] , jsons['fp_hm'])
        ...
        # 返回单张发票的详细信息
            ...
            result = self.DB.search_invoice(
                jsons['key'][:12] , jsons['key'][12:])
            result['cmd'] = 212
        ...
        # 接受用户普通查验请求
            ...
            if result == None or result['state'] == 0:
                ...
                back_send_dict = {
                    'fp_dm': result['fp_dm'] , 'fp_hm': result['fp_hm']}
                ...
                # 将发票放入发票夹
                self.DB.add_invoice(
                    back_send_dict['fp_dm'] , back_send_dict['fp_hm'])
        ...
        # 接受客户端验证码并查验
            ...
            if all_data['error'] == 'None':
                ...
                all_data['state'] = 1
                self.DB.add_invoice(
                    self.invo.data_hold['fp_dm'] , self.invo.data_hold['fp_hm'])
        ...
        # 发送图片邮件
            ...
            with open("C:\\Users\\Administrator\\Desktop\\invoice\\spider\\images\\" + jsons['fp_dm'] + jsons[
                'fp_hm'] + ".png" , "rb") as f:
                pic_data = f.read()
        ...
        # 发送EXCLE邮件
            ...
            for each in jsons['list']:
                ...
                # 插入信息
                if result:
                    move_lenght += ex.insert_data(kindsof(result) ,
                                                  num , move_lenght)
            # 保存文件
